...
@Author :   KarenJ
@Note   :   The Fitness content: an estimated disassemlby time (see more in file Fn_Fitness).
...
'''**************************************'''
# file_path = 'EDBA_EVB_Stage1_DataChuxuan.xlsx'
# file_path = 'EDBA_EVB_Stage2_DataChuxuan.xlsx'
file_path = 'EDBA_EVB_Stage3_DataChuxuan.xlsx'

'''Load the information of:
Disassembly tools;
Distance matrix;
Penalty of direction;
Penalty of tools.'''
dsp_tool_data = Fn.excel_to_array(file_path, 'DisassemblyTool')
dsp_distance_data = Fn.excel_to_array(file_path, 'DistanceMatrix')
...
for scout_bee in range(0, ba_scout_bees):
    ...
    [directionchange, toolchange, distance] = Fn3.penalty_accumulating(dsp_distance_data,
                                                                       dsp_penalty_direction,
                                                                       dsp_penalty_tool, sequence,
                                                                       direction, tool, cad_num)
    totalcost = distance / robot_speed + toolchange + directionchange * time_scale
    ...
    for elitesite in range(0, ba_elite_site):
        ...
        for elitebee in range(0, ba_elite_bees):
            ...
            [babybee_directionchange,
             babybee_toolchange,
             babybee_distance] = Fn3.penalty_accumulating(dsp_distance_data,
                                                          dsp_penalty_direction,
                                                          dsp_penalty_tool,
                                                          babybee_seq,
                                                          babybee_direct,
...
plt.title('EDBA_Python_RunForEVB3_6Directions_1DistanceMatrix\n80-4-1-1-2-1000_Runtime1')
