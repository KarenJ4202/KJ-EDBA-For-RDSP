...
'''ABBREVIATIONS: 
ds->disassembly; dir->direction; comp->component.
'''
...
def generate_feasible_sequence(mtx_sum, mtx_itf, num_comp):
    ...
    '''Randomly(temporarily) pick a component to remove it.'''
    ...
    '''Store the picked component to the sequence.'''
    sequence[0] = ds_comp[temp]

    '''Check the direction-freedom of the picked component.'''
    free = []
    for i in range(0, len(ds_comp)):
        if ds_comp[i] == ds_comp[temp]:
            free.append(ds_dir[i])

    '''From all the direciton-freedoms, randomly pick one as the disassembly direciton of picked component and store.'''
    free_dir = free[random.randint(0, len(free) - 1)]
    # print("The picked component has direction-freedom on: ", free)
    direction[0] = free_dir
    # print("The current disassembly sequence and direction is: \n", sequence, '\n', direction)
    # print("*"*88)

    for num in range(1, num_comp):
        '''Remove the information of disassembled component from the summation matrix.'''
...
generate_feasible_sequence(sum_data, itf_data, num_comp=9)
'''